<div style="border: 3px dashed #ed792c; padding: 0 15px 0 15px;color: black;font-size: 18px">
    ...
    <div style=" font-size: 14px;margin-top: 15px;">
        ...
        <p>The order <span style=" text-decoration: underline;font-weight: bold; font-size: 18px">{{$orderNumb}}</span>
            confirmed by <span style=" text-decoration: underline;font-weight: bold; font-size: 18px">{{$restaurant->name_en}}</span>
            restaurant</p>
        ...
        <p>Payment method: <b style="font-size:18px">{{ ($order->payment_method == 'cod_payment') ? "Cash On Delivery (COD)" : "Online Payment" }}</b></p>
        <p>Restaurant inform that order can be arrived at around <b style="font-size:18px">{{$data['confirm_time']}}</b></p>
        <p>Please take a rest and your food will be come as soon as possible</p>
        <p>You can leave your  <a href="{{ url('/restaurants/'. $res_slug .'/reviews?token='.$token.'&district='.$data['district_slug']) }}" style="text-decoration: none">
                <button style="
                        background: #E76715;
                        color: white;
                        border: none;
        ...
        <p>Many thanks and have a nice meal !!!</p>
        ...
        <br/>
        {{--<div class="width:100%">Sub total: {{CommonService::formatPriceVND($order->sub_total_amount) }}</div>--}}
        {{--<div class="width:100%">Shipping Fee: {{CommonService::formatPriceVND($order->shipping_fee) }}</div>--}}
        {{--<div class="width:100%">Total: {{ $order->totalAmountVND() }}</div>               --}}

        <div style="margin-top:20px">
            <h2 style="border-bottom: 1px solid #7c9bd6;border-top: 1px solid #7c9bd6;">Customer Info</h2>
        </div>
        <p><b>Mr/Ms: </b><span
                    style=" text-decoration: underline;font-weight: bold;">{{ $order_customer_info->full_name }}</span>
        </p>
        <p><b>Previous order: {{$previousOrder > 0 ? $previousOrder : 'No order'}}</b></p>
        <p><b>Email: {{$order_customer_info->email ?? '' }}</b></p>
        <p><b>Phone: {{$order_customer_info->phone ?? '' }}</b></p>
        <div style="margin-top:15px">
        ...
        <p>Address:
            ...
            {{ App\Models\Ward::find($order_delivery_info->ward_id)->type_en }} {{ App\Models\Ward::find($order_delivery_info->ward_id)->name_en }}
            , {{ App\Models\District::find($order_delivery_info->district_id)->type_en }} {{ App\Models\District::find($order_delivery_info->district_id)->name_en }}
            ,
        ...
        <p><b>Request
                time: {{!empty($order->delivery_time) ? date('d-m-Y H:i', strtotime($order->delivery_time)) : 'As soon as possible'}}</b>
        </p>
