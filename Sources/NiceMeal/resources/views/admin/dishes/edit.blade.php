...
@section('content')
    ...
    <div class="m-content">
        <div class="m-portlet">
            ...
            <div class="row">
                <div class="col-md-7">
                    {!! Form::open(['method'=>'PATCH','url' => ['/admin/'.$res->res_Slug.'/dishes',$dish->id], 'class' => 'm-form m-form--fit m-form--label-align-left', 'id' => 'submitForm']) !!}
                            {{ csrf_field() }}
                            @include ('admin.dishes.form',['dish'=>$dish,'categories'=>$categories])
                    {!! Form::close() !!}
                </div>
