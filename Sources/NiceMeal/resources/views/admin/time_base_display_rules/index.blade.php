    ...
    <div class="m-content">
        <div class="m-portlet m-portlet--mobile">
            <div class="m-portlet__body">
                {!! Form::open(['method' => 'GET', 'url' => '/admin/'.$res->res_Slug.'/time-base-display-rules', 'class' => '', 'role' => 'search'])  !!}
                <div class="m-form m-form--label-align-right m--margin-top-20 m--margin-bottom-30">
                    <div class="row align-items-center">
                                    ...
                                    <div class="m-form__group m-form__group--inline">
                                        ...
                                        <div class="m-form__control">
                                            <select class="form-control m-bootstrap-select" name="status"
                                                ...
                                                <option value="{{ \App\Models\TimeBaseDisplayRule::STATUS_FILTER['active'] }}" {{ ($status == \App\Models\TimeBaseDisplayRule::STATUS_FILTER['active'] ? 'selected' : '') }}>
                                                    @lang('admin.restaurants.statuses.active')
                                                </option>
                                                <option value="{{ \App\Models\TimeBaseDisplayRule::STATUS_FILTER['inactive'] }}" {{ ($status == \App\Models\TimeBaseDisplayRule::STATUS_FILTER['inactive'] ? 'selected' : '') }}>
                                ...
                                <div class="col-md-4">
                                    <div class="m-input-icon m-input-icon--left">
                                        <input type="text" class="form-control m-input" name="q"
                                               value="{{ Request::get('q') }}"
                                               placeholder="@lang('admin.time_base_display_rules.search.place_holder_text')"
                                               id="generalSearch">
                        ...
                        <div class="col-xl-4 order-1 order-xl-2 m--align-right">
                            <a href="{{ url('/admin/'.$res->res_Slug.'/time-base-display-rules/create?back_url='.$currentURL) }}"
                               class="btn btn-primary m-btn m-btn--custom m-btn--icon m-btn--air m-btn--pill">
                            <span>
                                <i class="la la-plus-circle"></i>
                                <span>
                                    @lang('admin.time_base_display_rules.createButton')
                                </span>
                            </span>
                            </a>
                ...
                <table class="table table-striped table-bordered table-responsive-md">
                    ...
                    <tr class="table-dark">
                        {{--<th>--}}
                            ...
                            {{--</label>--}}
                        {{--</th>--}}
                        <th>@lang('admin.time_base_display_rules.columns.name')</th>
                        <th>@lang('admin.time_base_display_rules.columns.active')</th>
                        <th>@lang('admin.time_base_display_rules.columns.action')</th>
                    </tr>
                    </thead>
                    <tbody class="m-datatable__body">
                    @foreach($timeBaseDisplayRules as $item)
                        <tr>
                            ...
                            <td class="text-nowrap align-middle">
                                <a href="{{ url('/admin/'.$res->res_Slug.'/time-base-display-rules/' . $item->rule_id . '/edit?back_url='.$currentURL) }}"
                                   class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill"
                                   title="@lang('admin.time_base_display_rules.tooltip_title.edit')">
                                    <i class="la la-edit"></i>
                                </a>
                                {!! Form::open([
                                   'method' => 'DELETE',
                                   'url' => ['/admin/'.$res->res_Slug.'/time-base-display-rules', $item->rule_id],
                                ...
                                <a href="javascript:void(0);"
                                   ...
                                   title="@lang('admin.time_base_display_rules.tooltip_title.delete')">
                                ...
                                {!! Form::open([
                                    'method' => 'POST',
                                    'url' => ['/admin/'.$res->res_Slug.'/time-base-display-rules/duplicate/'. $item->rule_id],
                                    'style' => 'display:inline'
                                 ]) !!}
                                <a href="javascript:void(0);"
                                   class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill"
                    ...
                    @endforeach
                    @if(count($timeBaseDisplayRules) == 0)
                        <tr>
                            <td colspan="100%">
                                <i><h6>@lang('admin.time_base_display_rules.not_found')</h6></i>
                            </td>
                        </tr>
                    @endif
                    </tbody>
                </table>
                {{--{!! Form::open(['method' => 'POST', 'url' => '/admin/'.$res->res_Slug.'/time-base-display-rules/batch', 'class' => ''])  !!}--}}
                {{--<div class="row ml-3">--}}
                    {{--<div class="col-xl-3 mt-2">--}}
                        {{--<label class="m-checkbox m-checkbox--solid m-checkbox--info">--}}
                            {{--<input name="all_elements" type="checkbox"> All elements ({{ \App\Models\TimeBaseDisplayRule::where('restaurant_id','=',$res->id)->count() }})--}}
                            {{--<span></span>--}}
                        {{--</label>--}}
                    {{--</div>--}}
                    {{--<div class="col-xl-2">--}}
                ...
                {{--</div>--}}
                {{--{!! Form::close() !!}--}}
                @if(!empty($timeBaseDisplayRules))
                    {!! Form::open(['method' => 'GET', 'url' => '/admin/'.$res->res_Slug.'/time-base-display-rules', 'class' => '', 'role' => 'search'])  !!}
                    <div class="m-datatable m-datatable--default m-datatable--brand m-datatable--loaded">
                        <div class="m-datatable__pager m-datatable--paging-loaded clearfix">
                            {!! $timeBaseDisplayRules->appends(['q' => Request::get('q'), 'status' => $status])->render() !!}
                        </div>
                    </div>
                    {!! Form::close() !!}
                @endif
    ...
    <script>
        ...
        function changeStatus(item_id){
            ...
            if(!requesting[item_id])
            {
                ...
                $.ajax({
                    url: "{{url('admin/'.$res->res_Slug.'/time-base-display-rules/changeStatusTimeBaseDisplay')}}",
                    type: "post",
                    data:{
                        '_token': "{{csrf_token()}}",
                        'item_id': item_id,
                    ...
                    success:function(response){
                        if(response.error){
                        toastr.error("@lang('admin.time_base_display_rules.time_base_display_rule_status.error')");
                        }else{
                            requesting[item_id] = false;
