        ...
        <table class="table table-striped table-bordered table-responsive-md">
            ...
            <draggable :list="promotionsNew" :options="{animation:150}" :element="'tbody'" @change="update">
                <tr class="text-center move-cursor" v-for="(item, index) in promotionsNew">
                    <td v-if="item.image">
                        <img width="40" height="40" class="img-circle" style="border: 1px solid #ddd;" v-bind:src="'/images/promotions/'+item.image">
                    </td>
                    <td v-else>
                        <img width="40" height="40" class="img-circle" style="border: 1px solid #ddd;" v-bind:src="'/common-assets/img/promotion_image.jpg'">
                    </td>
                    <td>{{ item.name_en }}</td>
                    <td>{{ item.name_vi }}</td>
                    <td>{{ item.name_ja }}</td>
