app.controller('RestaurantInfoCtrl', function($scope, $http, $timeout, $interval, cart) {
    $scope.init = function() {
        $timeout(function() {
        ...
        $scope.printAvailDateInWeek = function(time_setting){
          var returnStr = "";
          for(date in $scope.dateInWeek){
            if(time_setting[$scope.dateInWeek[date]]){
                returnStr += $scope.dateName[$scope.dateInWeek[date]] + ',';
            }
